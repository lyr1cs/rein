...
**Current release: `v0.32.0`** (2026-05-18) — Trust & Measurement Phase 2 (eval-gate harness). New `eval::gates` module exposes `GateScorecard` / `Gate` trait / `compare_scorecards` (8-rule classification pipeline: presence, schema version, gate identity, kind, run freshness, stub, duplicate-id, strict id-set + paired McNemar non-inferiority). Recall gate ships full-impl over a 20-fixture corpus seeded into an isolated in-memory `SqliteStore` per fixture (deterministic, hermetic, no live LLM/embedding required); dedup / admission / latency ship as `NoData` stubs (full implementations queued for v0.33+). New `rein-eval gate {baseline,run,compare,status}` CLI subcommand; `rein_trust_measurement` now reads real scorecards (was 4 hard-coded placeholder strings); `rein doctor` adds `check_eval_gates` surfacing stale baselines, Bail comparisons, and corrupt scorecards as WARN. Reference baseline committed at `docs/eval-baselines/recall.json` (score = 1.000 at v0.32.0). Tests: 1647 passed / 5 ignored / 0 failed. See the [v0.32.0 release](https://github.com/lyr1cs/rein/releases/tag/v0.32.0).

**Previous release: `v0.31.4`** (2026-05-17) — attribution metadata switched to the maintainer's GitHub handle across Cargo workspace authors, DXT / plugin / marketplace manifests, LICENSE / README / CONTRIBUTING copyright lines, and git commit identity. Metadata-only patch, no source / behavior / test changes. See the [v0.31.4 release](https://github.com/lyr1cs/rein/releases/tag/v0.31.4).
...
1. Download `rein-v0.32.0.mcpb` from
   [the v0.32.0 release](https://github.com/lyr1cs/rein/releases/download/v0.32.0/rein-v0.32.0.mcpb).
2. Clear macOS quarantine (one-time, the build is unsigned):

   ```bash
   xattr -d com.apple.quarantine ~/Downloads/rein-v0.32.0.mcpb
   ```
...
cargo install --git https://github.com/lyr1cs/rein --tag v0.32.0 --locked rein
```
...
The v0.21 → v0.32.0 arc rebuilt rein around four axes: a unified operation registry, an adaptive read-side synthesis (ARS) stack with feedback-driven gates, secure remote MCP deployment for Claude clients, and a reproducible eval-gate harness for Trust & Measurement.

| Version | Theme | Highlights |
|---|---|---|
| **v0.32.0** (2026-05-18) | Trust & Measurement Phase 2 — eval-gate harness | New `eval::gates` module: `GateScorecard` / `Gate` trait / `compare_scorecards` 8-rule pipeline (presence / schema / identity / kind / freshness / stub / duplicate-id / strict id-set + paired McNemar non-inferiority). Recall gate ships full-impl over a 20-fixture corpus seeded into hermetic in-memory `SqliteStore` per fixture (deterministic, no live LLM/embedding). Stub gates queued for v0.33+: dedup / admission / latency (all return `NoData`). New `rein-eval gate {baseline,run,compare,status}` CLI subcommand; `rein_trust_measurement` reads real scorecards; `rein doctor` surfaces stale baselines / Bail / mis-wired / corrupt as WARN. Reference baseline at `docs/eval-baselines/recall.json` (score = 1.000). **1647 tests / 0 fail / 5 ignored.** |
...
v0.21 → v0.32.0 这一段重构围绕四条主线：统一 operation registry、ARS（Adaptive Read-Side Synthesis）反馈驱动闸门栈、面向 Claude 客户端的安全远程 MCP 部署，以及可重复 Trust & Measurement eval-gate 闸口。

| 版本 | 主题 | 重点 |
|---|---|---|
| **v0.32.0** (2026-05-18) | Trust & Measurement Phase 2 — eval-gate harness | 新 `eval::gates` 模块：`GateScorecard` / `Gate` trait / `compare_scorecards` 8 规则流水线（presence / schema / identity / kind / freshness / stub / duplicate-id / strict id-set + 配对 McNemar 非劣效）。Recall gate 跑全量实现，20 fixture 注入隔离的 in-memory `SqliteStore`（确定性、hermetic、无 LLM/embedding API 调用）。Stub gates 占位排到 v0.33+：dedup / admission / latency 均返 `NoData`。新 `rein-eval gate {baseline,run,compare,status}` CLI 子命令；`rein_trust_measurement` 改读真实 scorecard；`rein doctor` 把过期 baseline / Bail / 错装 / corrupt 暴露为 WARN。参考 baseline 在 `docs/eval-baselines/recall.json`（v0.32.0 score = 1.000）。**1647 测试 / 0 失败 / 5 ignored**。 |
