...
|---|---|---|
| **v1.4.0** (2026-09-04) | Runway automation | **Pipeline observability**: every adaptive stage is timed and persisted (`adaptive_pipeline_last_run`), surfaced by `rein doctor`, `adaptive-status` and a `rein gc` summary line; a killed run stays visible as `running`. **A12 publish-on-drift**: a calibration whose store moved mid-run is published as `Complete` carrying its build fingerprint instead of being discarded — the live resolver still refuses a stale generation on the input epoch. **A12 LOO exclusion index**: hash + posting lists reproduce `extract::dedup::similarity` exactly (randomized property test + real-database equivalence run); corpus build 2 h → 8 s. **Vector durability**: `rein warmup` backfills missing `vec_memories` rows, migrates the table atomically on a model change (staged, revalidated at publish, rows that raced the swap flagged for re-embedding), and a `vec_rows_provenance` stamp refuses mixed-model writes (`rein warmup --trust-existing-vectors` attests a legacy table). **Claude Code prompt recall**: `[hooks.claude]` with `prompt_context_source = "recall"` runs a local fast recall, emits `RecallComplete` and appends a `rein_feedback` attribution line; doctor `claude_hooks`, `vector_provenance` and `adaptive_pipeline_last_run` checks. `config_version` 4. **1887 lib tests / 0 fail; clippy + fmt clean; 26 adversarial review rounds.** |
| **v1.3.0** (2026-07-15) | Self-supervised activation | **#A12 recall-fusion activation infrastructure** (default fail-closed, shadow until promoted): leave-one-evidence-out calibration from structural families (canonical evidence / concept / episode), SHA-256 family-disjoint 5-fold split with a permanent activation holdout fold, side-effect-free `recall_loo_trace`, family-equal optimizer + paired top-3 McNemar holdout gate, sealed versioned calibration state + durable `a12_input_epoch` coherence (schema v4, 21 triggers, per-recall re-verification). **Judge structural calibration**: 4 deterministic probe kinds, per-surface versioned state, `[ars.llm_judge.structural_anchors]` (`off` \| `monitor` \| `enforce`, default `off`); zero human pairs never fabricates a human κ. **#C2 dedup calibration safety**: destructive lexical merges floored at the static threshold, exact one-sided Clopper-Pearson false-merge bound (zero-failure 2% @ 95% needs ≥ 149 sealed negatives → current n = 10 reports `NoData`). `config_version` 3; `ars_parameter_policy` schema 3. **1819 lib tests / 0 fail; 27 workspace suites green; clippy + fmt clean; 7 adversarial review rounds.** |
...
**当前版本：`v1.4.0`**（2026-09-04）— runway 自动化线。自适应 pipeline 每个阶段的耗时与结果写入 SQLite（`adaptive_pipeline_last_run`，由 `rein doctor` / `adaptive-status` / `rein gc` 呈现）；A12 在多小时校准期间库发生变化时照常发布 `Complete` 代（激活侧仍按 input epoch 严格 fail-closed）；leave-one-out 语料构建改用倒排索引，语义与逐对比较完全一致（9.7k 记忆库上 2 小时降到 8 秒，并在真实库上与参考实现逐条比对验证）；`rein warmup` 回填缺失的 `vec_memories` 行，模型变更时原子换表，`vec_rows_provenance` 戳拒绝混模型写入；`[hooks.claude]` 让 Claude Code 在 prompt 时注入真实召回上下文并发出带反馈 id 的 `RecallComplete`。`config_version` 4。**1887 lib 测试 / 0 失败；clippy + fmt clean；26 轮对抗式审查（修复 14 个 P1 + 43 个 P2，最后两轮零发现）。** License: AGPL-3.0-or-later。

<details>
<summary>v1.3.0（2026-07-15）— 自监督激活线</summary>

A12 召回融合权重现在可以**不依赖人工反馈**完成校准，全程 fail-closed：结构化 leave-one-evidence-out family（canonical evidence / concept / episode）、SHA-256 family 隔离切分 + 永久激活 holdout、无副作用召回 trace、配对 McNemar holdout 闸门、sealed 参数策略（schema 3）——自动证据只能以每轮 ≤0.05 步进 `recall_fusion:*`，且必须通过 judge 结构信任门。SQLite schema v4 的 input-epoch 计数器把每次校准绑定到训练时的精确库状态；结构化 judge 探针让零人工配对部署也有诚实的健康门；破坏性 dedup 合并钉死在静态阈值之上并配精确假合并安全上界。**Released ≠ activated**：所有能力默认 shadow/fail-closed，真实证据过 release gate 才会激活。1819 lib 测试 / 0 失败；clippy + fmt clean；7 轮对抗审查。License: AGPL-3.0-or-later。详见下方[最近版本](#最近版本)。
...
| 版本 | 主题 | 重点 |
|---|---|---|
| **v1.4.0** (2026-09-04) | Runway 自动化 | **Pipeline 可观测**：每个自适应阶段计时并持久化（`adaptive_pipeline_last_run`），由 `rein doctor` / `adaptive-status` / `rein gc` 摘要行呈现；被杀掉的运行以 `running` 状态可见。**A12 漂移仍发布**：跑期间库发生变化的校准以自身 build fingerprint 发布为 `Complete` 而非整份丢弃——激活端仍按 input epoch 拒绝过期代。**A12 LOO 倒排索引**：hash + posting list 精确复现 `extract::dedup::similarity`（随机 property test + 真实库等价验证）；语料构建 2 小时 → 8 秒。**向量持久化**：`rein warmup` 回填缺失 `vec_memories` 行，模型变更时原子换表（staging + 发布时重验证，换表期间竞争写入的行标记重嵌入），`vec_rows_provenance` 戳拒绝混模型写入（`rein warmup --trust-existing-vectors` 认证历史表）。**Claude Code prompt 召回**：`[hooks.claude]` 配 `prompt_context_source = "recall"` 走本地快速召回，发出 `RecallComplete` 并追加 `rein_feedback` 归因行；doctor 新增 `claude_hooks` / `vector_provenance` / `adaptive_pipeline_last_run` 检查。`config_version` 4。**1887 lib 测试 / 0 失败；clippy + fmt clean；26 轮对抗式审查。** |
| **v1.3.0** (2026-07-15) | 自监督激活 | **#A12 recall-fusion 激活基础设施**（默认 fail-closed，promote 前保持 shadow）：从结构 family（canonical evidence / concept / episode）派生 leave-one-evidence-out 校准，SHA-256 family-disjoint 5-fold 切分 + 永久激活 holdout fold，无副作用 `recall_loo_trace`，family-equal 优化器 + 配对 top-3 McNemar holdout 闸，密封版本化校准状态 + 持久 `a12_input_epoch` 一致性（schema v4、21 个 trigger、每次 recall 重验证）。**Judge 结构校准**：4 种确定性 probe、per-surface 版本化状态、`[ars.llm_judge.structural_anchors]`（`off` \| `monitor` \| `enforce`，默认 `off`）；零 human pair 绝不伪造 human κ。**#C2 dedup 校准安全**：破坏性词法合并以静态阈值为下限，精确单侧 Clopper-Pearson 假合并上界（零失败 2% @ 95% 需 ≥ 149 sealed negatives → 当前 n = 10 报 `NoData`）。`config_version` 3；`ars_parameter_policy` schema 3。**1819 lib 测试 / 0 失败；27 个 workspace suite 全绿；clippy + fmt clean；7 轮对抗式审查。** |
| **v1.0.0** (2026-05-31) | 1.0 freeze + #A5 三元组持久化 | 稳定面冻结：baseline schema 在迁移框架后冻结（`baseline_schema_is_frozen`）、40 个 MCP 工具 arg-schema 钉死（`mcp_tool_arg_surface_is_frozen`）、MSRV 钉到 Rust 1.86（CI `msrv` job；三处 `is_multiple_of`/1.87 改 `%`）、`config_version` 键 + load-time downgrade guard、`/v1/*` REST 版本化别名（header-token 认证；GUI cookie 仍 `Path=/api`）。头条功能 **#A5 三元组持久化**——第一条真实前向迁移（`memory_triples`，schema v2），由默认关的 `[dedup].persist_triples` flag 门控；默认路径逐位一致。96 个 config 字段补文档。**1470 lib 测试 / 0 失败；clippy + fmt clean；agent-team 审计 clean（PII / 正确性 / hygiene / docs）**。 |
...
| **v0.31.4** (2026-05-17) | Attribution metadata | 所有 maintainer attribution（Cargo authors、LICENSE / README / CONTRIBUTING copyright、DXT / plugin / marketplace 清单、git config `user.name`）切到 maintainer GitHub handle。仅元数据，无源码 / 行为 / 测试变化。 |
| **v0.31.0 → v0.31.3** (2026-05-16/17) | OAuth 安全 + recovery-paths + 构建路径卫生 | 4 个 release。**v0.31.0**: OAuth A-H1 JWT `kid` 严格匹配（关 rotation overlap 伪造）、A-H2 `migrate_oauth_tables` schema-version gate + 崩溃恢复 NULL 清理、A-H3 `verify_bearer` 30s SHA-256-keyed cache + 60s 去抖 `mark_client_used` + pool-backed fast path + 5 处 cache invalidation。**v0.31.1**: D1 锁内原子 gen 复检 + D2 bearer cache key DB-identity scoping。**v0.31.2**: D1 `TantivyFts::open_existing` non-creating recall variant + D3 symlink chain 环检测（256-hop 上限）+ D4 过期 Tantivy `.rebuilding` marker TTL 恢复 + D5 `atomic_write_string` chown 保留。**v0.31.3**: `scripts/build-dxt.sh` `CARGO_ENCODED_RUSTFLAGS` 配 `--remap-path-prefix=$HOME=user`，让 `file!()` macro 展开不再把 builder 路径烤进 release binary。 |
| **v0.30.0 → v0.30.2** (2026-05-10/12) | 内建 OAuth provider + recall 启动 warmup | **v0.30.0**: 显式 `[server].auth` policy 与单用户 OAuth provider（面向 Claude Cowork / claude.ai / mobile remote MCP）—— Authorization Server metadata、DCR、Authorization Code + PKCE S256、refresh rotation、revocation、SQLite OAuth clients/grants/signing keys、owner approval GUI、Connectors 管理 UI、OAuth GC、doctor 检查。通过 Cloudflare Quick Tunnel 端到端验证。**v0.30.1**: `/mcp` 全部 4xx 改 JSON-RPC 2.0 error envelope，让 claude.ai 显示真实 rejection reason 而不是不透明的 "An unknown error occurred"；`rein doctor` `oauth_provider` 在 `auth_policy = "oauth"` + clients registered + 0 active grants 时 WARN。**v0.30.2**: P0 recall 启动失败修复 —— warmup B1（同步 Tantivy rebuild 阻塞 recall） / B2（破坏性 `remove_dir_all`） / B4（冷启动无条件 rebuild） / B5（`.rebuilding` 生命周期无恢复）链条造成用户看到 "rein won't start"，尽管进程级 boot 已成功。23 轮 codex audit；5 个 corner case 延到 v0.31.x。 |
...
**Copyright (C) 2026 lyr1cs.** 保留所有权利，除非依 AGPL-3.0-or-later 授权。
...
如果需要非 AGPL 的 license 用于商业 / 闭源场景，本项目 copyright holder（`lyr1cs`）保留 dual-license 权利——开 issue 联系。
