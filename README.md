...
| Feature | Description |
|---------|-------------|
| **35 MCP tools** | 8 core memory + 9 maintenance + 10 knowledge graph + 2 temporal + 2 adaptive + 1 session ingest + 2 ARS concept-summary (v0.24) + 1 Cap C archival summary refresh (v0.26) + 1 GUI/MCP synthesis interaction feedback (v0.26). v0.25 extends `rein_recall` with opt-in `synthesize=true` for recall-time LLM narrative synthesis (Cap B); v0.26 adds Cap C cold-tier archival summaries + D direction synthesis-feedback events flowing into M1; v0.26.2 closes 32 audit findings (8 HIGH + 8 MEDIUM original + 16 audit-cycle additions across 5-agent fan-out + 11 Codex review rounds): auth default-deny, recall correctness with status-aware SQL filters + canonical-first preservation of superseded rows, `apply_evolution` side-index discipline, full backend↔GUI synthesis bucket round-trip, archival_summary lifecycle in `update()`, M5 strip vec/HNSW invalidation. |
