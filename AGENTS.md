...
rein v1.3.0 — Current released baseline. Rust single binary (**MSRV: Rust 1.86**) with a frozen 40-tool MCP surface, config schema version 3, and database schema version 4. v1.3.0 is the self-supervised activation line: A12 recall-fusion calibration (structural LOO families + permanent holdout + sealed `ars_parameter_policy` schema 3 + fail-closed canary), input-epoch coherence triggers, structural Judge calibration, and C2 dedup-calibration safety — all default fail-closed/shadow. Released ≠ activated: production weight activation still requires accumulated live evidence plus the release gate.
