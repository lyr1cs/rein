...
rein v1.4.0 — Current released baseline. Rust single binary (**MSRV: Rust 1.86**) with a frozen 40-tool MCP surface, config schema version 4, and database schema version 4. v1.4.0 is the runway automation line: the adaptive pipeline records every stage to `adaptive_pipeline_last_run` (doctor / `adaptive-status` / `rein gc`), A12 publishes a calibration whose store moved mid-run instead of discarding it (activation stays fail-closed on the input epoch), the leave-one-out corpus build uses an exclusion index with identical semantics (hours to seconds), `rein warmup` backfills `vec_memories` rows behind a `vec_rows_provenance` stamp that refuses mixed-model writes, and `[hooks.claude]` gives Claude Code prompt-time recall context. v1.3.0 remains the self-supervised activation line underneath (A12 structural LOO calibration, input-epoch coherence, structural Judge calibration, C2 dedup safety) — all default fail-closed/shadow. Released ≠ activated: production weight activation still requires accumulated live evidence plus the release gate.
