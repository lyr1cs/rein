...
rein v0.28.17 — Multi-source cross-validated memory MCP server for AI agents. Rust single binary. 40 MCP tools (v0.28.6 added `rein_trust_measurement`; v0.28.4 added `rein_ars_acceleration_gate`; v0.28.7 through v0.28.17 hardened existing surfaces without adding tools). **v0.28.17** is a diagnostics patch for the Cowork/auth incident class: `run_http` now warns when `REIN_HTTP_TOKEN` is set while `[server].allow_unauthenticated_loopback=true` because token auth wins and the loopback flag is a runtime no-op; `rein doctor` now reports the same conflict as `http_auth` WARN; `rein doctor` also checks Codex's `~/.codex/config.toml` Rein MCP entry and warns when it points at a non-loopback HTTP URL, since recall may then hit a different machine/database than local `rein` CLI. **v0.28.16** closed Codex 0.129 `[mcp.<name>]` → `[mcp_servers.<name>]` compatibility, completing the v0.28.15 hooks-gate rename patch. **v0.28.11** is a docs-only polish on v0.28.10 adding the verified end-to-end Tailscale Funnel walkthrough (FQDN discovery / `[server].allowed_hosts` / `env -u REIN_HTTP_TOKEN` shell-inheritance trap / `tailscale funnel --bg --https=443` modern syntax / Let's Encrypt provisioning wait / `tail -F /tmp/rein-sse.log` as ground truth / Claude Desktop UI Add Custom Connector flow) that brought up a working Cowork connector during v0.28.11 development; plus a "Common pitfalls" subsection enumerating the four most-likely operator traps (env-var inheritance, plugin-marketplace-vs-custom-connector coexistence, LAN-side `*.ts.net` interception, cert provisioning delay) and a `.gitignore` defense-in-depth patch blocking `*.crt` / `*.key` / `*.pem` / `*.p12` / `*.pfx` from accidentally being committed (defends against `tailscale cert` writing private keys into cwd). **v0.28.10** was a docs-only patch on v0.28.9 closing the remote-MCP gap for Claude Cowork / claude.ai / mobile via a new deployment guide at `docs/manual/02b-remote-mcp-deployment.md` (4 recipes: Cloudflare Tunnel default, Tailscale Funnel, Caddy/nginx + Let's Encrypt, ngrok dev). README install table extended to 4 paths (Chat tab DXT / Claude Code marketplace / Cowork remote MCP / from source). Binary unchanged from v0.28.9 — rein already implements the Streamable HTTP transport at `/mcp` via `rmcp::transport::streamable_http_server::StreamableHttpService` (`crates/rein/src/mcp/server.rs:588`). **v0.28.9** was a distribution-channels patch on v0.28.8: adds Claude Desktop one-click install via DXT (`.mcpb` artifact, macOS Apple Silicon) and Claude Code plugin marketplace registration. Build pipeline at `scripts/build-dxt.sh`; manifest at `dxt/manifest.json`; plugin manifests at `.claude-plugin/marketplace.json` + `plugins/rein/`. Maintainer guide at `docs/guides/dxt-build.md`; ADR at `docs/decisions/distribution-channels.md`. v0.28.8 was a second-pass audit hardening on v0.28.7: 17 codex review rounds, 15 P2 + 1 P3 closed, 0 P1 throughout, 2-consecutive-clean saturation at R16/R17. Default-OFF runtime behavior bit-identical to v0.28.7 — only `[ars.acceleration]` defaults `true`; `[ars.llm_judge]` + `[ars.llm_judge.nightly_cron]` stay default-off per the v0.28 charter Non-Goal. Live learned parameters still fail closed behind a healthy canary `ars_parameter_policy` row plus positive scoped adoption weights; runtime adoption remains scoped per parameter, surface, query type, and cluster; SignalHint feedback remains active outside shadow mode; release-gate output exposes scoped weights; and Trust & Measurement reports release gate, eval gates, index consistency, background observability, and active-learning status. Unified operation registry (CLI / MCP / REST authored once via `#[op]` macro). Self-adaptive engine (M1-M6), 3-channel retrieval (FTS + Vector + KG), transparent LLM proxy, async memory pipeline, unified dedup architecture, canonical-first read model, evidence-aware recall, hybrid CJK tokenization, cluster-aware admission, service management, and Neural Wiki GUI remain core surfaces.
