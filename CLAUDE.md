...
rein is a multi-source cross-validated memory MCP server (**40 MCP tools** as of v0.28.6). v0.28.6 completes ARS acceleration rollout: acceleration, runtime LLM judge, and nightly calibration default on while learned runtime behavior fails closed behind `ars_parameter_policy`; adoption weights are scoped by parameter/surface/query/cluster; SignalHint feedback remains active in canary mode; and `rein_trust_measurement` exposes release gate, eval gates, index consistency, background observability, and active-learning status. Evidence-aware canonical-first memory flow). Key modules:
