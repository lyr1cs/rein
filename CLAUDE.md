...
rein is a multi-source cross-validated memory MCP server (**38 MCP tools** as of v0.27.1, verified by python3 grep over `#[op(... mcp(name = "..."))]`: `total=47 mcp-visible=38`. v0.27.1 adds the runtime LLM judge — opt-in via `[ars.llm_judge].enabled` — that hooks at synthesis (`ops/recall_synthesis.rs`) and concept-summary (`ops/concept_summary.rs`) mint time; auto-enqueue is sample-rate Bernoulli, manual MCP rehydrates from `<resolve_buffer_dir>/queue/<db_hash>/synthesis_cache.jsonl`. v0.27.0 added `rein_feedback_concept_summary` (Cap A mirror feedback); v0.26.2 ships a 32-bug security + correctness hotfix closing 8 HIGH + 8 MEDIUM from a user-driven Codex audit on v0.26.1 plus 16 audit-cycle additions across 11 follow-up `codex review --uncommitted` rounds: auth default-deny via `http_request_needs_auth(method, path, gui_enabled)` closes the `POST /not-mcp` MCP-service bypass; recall correctness via SQL `status IN ('active','updated')` filter on every FTS+vec path + defensive `memory_map.retain` for Tantivy/KG/episode + superseded-row preservation so `collapse_to_canonicals` maps to live canonical successors; `apply_evolution` side-index discipline (in-savepoint vec delete + post-RELEASE Tantivy/HNSW removal + DB-only refine update); synthesis bucket round-trip via `RecallSynthesisOutcome.{query_type, cluster_id}` carried backend → REST → GUI metadata so the D-direction adaptive gate works on GUI traffic; `update()` archival lifecycle clears all 6 archival_summary cols on semantic content change and drops `cold_archive` row only when `content` itself changes; M5 strip bypasses `update()` to preserve just-inserted cold_archive row. v0.26.1 wires the D-direction `query_type` from the classifier into the per-query synthesis gate and adds `[ars].synthesis_cold_start_n` plus a `rein-eval cold_archive` subcommand; v0.26.0 adds `rein_archive_summary_refresh` for ARS Capability C cold-tier archival summary, plus a D-direction event variant `SynthesisInteraction` flowing into a `synthesis_feedback` M1 consumer and a per-query adaptive synthesis-decision gate; v0.25.1 ships the Cap B eval harness (`rein-eval synthesis {baseline,run,compare}`) + Synthesis Lab GUI page + GUI HIGH-finding fixes + Brain/Graph polling refactor; v0.25.0 extends `rein_recall` with opt-in `synthesize=true` for recall-time LLM narrative synthesis (Cap B) without adding a new tool; v0.24.0 introduced ARS Cap A concept living-summary; all ARS feature flags default off. Evidence-aware canonical-first memory flow). Key modules:
