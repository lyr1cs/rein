...
pub fn classify(query: &str, has_time_from: bool, has_time_to: bool) -> QueryStrategy {
    ...
    if is_temporal(&lower) {
        ...
        let days_back = if is_relative { Some(extract_temporal_days(&lower)) } else { None };
...
fn is_temporal(lower: &str) -> bool {
    const TEMPORAL_EN: &[&str] = &[
        "when did", "when was", "what changed", "what happened",
        "last week", "last month", "yesterday", "recently", "recent",
        "this week", "this month", "changed since", "history of",
        "timeline", "before march", "after january", "since last",
    ];
    ...
    let has_date = lower.chars().zip(lower.chars().skip(4)).any(|(a, b)| a.is_ascii_digit() && b == '-');
...
fn extract_temporal_days(lower: &str) -> i64 {
    if lower.contains("yesterday") || lower.contains("昨天") { return 1; }
    if lower.contains("last week") || lower.contains("this week") || lower.contains("上周") { return 7; }
    if lower.contains("last month") || lower.contains("this month") || lower.contains("上个月") { return 30; }
    if lower.contains("recently") || lower.contains("最近") { return 14; }
...
fn is_exact_keyword(query: &str, lower: &str, word_count: usize) -> bool {
    ...
    if word_count <= 3 && is_mostly_ascii
        && !lower.starts_with("what") && !lower.starts_with("how")
        && !lower.starts_with("why") && !lower.starts_with("where")
        && !lower.starts_with("summarize") && !lower.starts_with("tell")
        && !lower.starts_with("everything") {
        return true;
    }

    // Code patterns: snake_case, CamelCase, ::, .rs, fn, struct
    if query.contains("::") || query.contains(".rs") { return true; }
    if lower.starts_with("fn ") || lower.starts_with("struct ") { return true; }

    // Predominantly snake_case or CamelCase (check if most characters follow code pattern)
    if word_count <= 4 {
        let code_words = query.split_whitespace()
            .filter(|w| w.contains('_') || (w.len() > 2 && w.chars().any(|c| c.is_uppercase()) && w.chars().any(|c| c.is_lowercase())))
            .count();
        if code_words > 0 && code_words >= word_count / 2 { return true; }
    }
...
fn is_exploratory(lower: &str) -> bool {
    const EXPLORATORY_EN: &[&str] = &[
        "what do i know", "what have i", "everything about",
        "tell me about", "summarize", "overview", "all about",
        "what do we know",
    ];
    const EXPLORATORY_ZH: &[&str] = &[
        "我知道什么", "关于", "总结", "概述", "所有",
    ];
    ...
    for pat in EXPLORATORY_ZH {
        if lower.contains(pat) && lower.len() > pat.len() + 6 { return true; }
    }
...
mod tests {
    ...
    fn test_temporal_queries() {
        assert_eq!(classify("when did the API change?", false, false).query_type, QueryType::Temporal);
        assert_eq!(classify("what happened last week", false, false).query_type, QueryType::Temporal);
        assert_eq!(classify("yesterday's decisions", false, false).query_type, QueryType::Temporal);
        assert_eq!(classify("什么时候改了", false, false).query_type, QueryType::Temporal);
        assert_eq!(classify("changes since 2026-03-20", false, false).query_type, QueryType::Temporal);
    }
    ...
    fn test_temporal_days_back() {
        assert_eq!(classify("yesterday's work", false, false).temporal_days_back, Some(1));
        assert_eq!(classify("what happened last week", false, false).temporal_days_back, Some(7));
        assert_eq!(classify("last month summary", false, false).temporal_days_back, Some(30));
    }

    #[test]
    fn test_exact_keyword() {
        assert_eq!(classify("ownership rules", false, false).query_type, QueryType::ExactKeyword);
        assert_eq!(classify("SqliteStore", false, false).query_type, QueryType::ExactKeyword);
        assert_eq!(classify("fn recall_temporal", false, false).query_type, QueryType::ExactKeyword);
        assert_eq!(classify("store::sqlite.rs", false, false).query_type, QueryType::ExactKeyword);
    }

    #[test]
    fn test_exploratory() {
        assert_eq!(classify("what do I know about Rust", false, false).query_type, QueryType::Exploratory);
        assert_eq!(classify("everything about the search pipeline", false, false).query_type, QueryType::Exploratory);
        assert_eq!(classify("tell me about memory management", false, false).query_type, QueryType::Exploratory);
    }

    #[test]
    fn test_semantic_default() {
        assert_eq!(classify("how does memory management relate to performance", false, false).query_type, QueryType::Semantic);
        assert_eq!(classify("connection pooling strategies for high throughput", false, false).query_type, QueryType::Semantic);
    }
    ...
    fn test_alpha_ordering() {
        ...
        let semantic = classify("things related to caching strategies and optimization", false, false);
